...
void drawTriangleNew(Vector2 v0, Vector2 v1, Vector2 v2, Color c0, Color c1, Color c2)
{
    ...
    for (int y = minY; y <= maxY; y++)
    {
        ...
        for (int x = minX; x <= maxX; x++)
        {
            ...
            if (inside)
            {
                ...
                Color color = { (unsigned char)r, (unsigned char)g, (unsigned char)b, (unsigned char)255 };
...
void drawTriangleOld(Vector2 v0, Vector2 v1, Vector2 v2, Color c0, Color c1, Color c2)
{
    int minX = (int)std::floor(std::min({ v0.x, v1.x, v2.x }));
    int minY = (int)std::floor(std::min({ v0.y, v1.y, v2.y }));
...
int main()
{
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Software Rasterizer");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose())
    {
        ...
        ImGui::Begin("Toggle Performance");

        ImGui::Checkbox("Optimized Rasterization: ", &toggle);

        if (toggle)
        {
            drawTriangleNew(tri1[0], tri1[1], tri1[2], tri1Colors[0], tri1Colors[1], tri1Colors[2]);
            drawTriangleNew(tri2[0], tri2[1], tri2[2], tri2Colors[0], tri2Colors[1], tri2Colors[2]);
        }
       
        else
        {
            drawTriangleOld(tri1[0], tri1[1], tri1[2], tri1Colors[0], tri1Colors[1], tri1Colors[2]);
            drawTriangleOld(tri2[0], tri2[1], tri2[2], tri2Colors[0], tri2Colors[1], tri2Colors[2]);
        }
